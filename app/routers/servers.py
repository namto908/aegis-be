...
from app.models.db_models import ServerDB, UserDB
...
def get_all_servers(
    ...
    """GET /api/servers - Retrieve list of all servers for current user"""
    servers = db.query(ServerDB).filter(ServerDB.user_id == current_user.id).all()
    return servers


@router.post("", response_model=ServerStatusItem, status_code=status.HTTP_201_CREATED)
def create_server(
    server_in: ServerStatusCreate,
    db: Session = Depends(get_db),
    current_user: UserDB = Depends(get_current_user)
):
...
def update_server(
    server_id: str,
    server_in: ServerStatusUpdate,
    db: Session = Depends(get_db),
    current_user: UserDB = Depends(get_current_user)
):
    """PUT /api/servers/{server_id} - Update server status / details"""
    db_server = db.query(ServerDB).filter(
        ServerDB.id == server_id,
        ServerDB.user_id == current_user.id
    ).first()
    if not db_server:
